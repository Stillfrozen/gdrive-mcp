...
Сервер запрашивает широкие scope: полный Drive, Docs и Sheets. Это нужно, чтобы читать **любой** доступный вам файл, а не только созданные этим приложением. Токен = весь Диск этого Google-аккаунта. Не авторизуйте им админский корпоративный логин, если достаточно отдельного аккаунта или узкой папки.
